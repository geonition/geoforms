...
def questionnaire(request, questionnaire_id, template='', no_save=''):
    ...
    use_cache = False
    use_cache_conditions = [
            not getattr(settings,'DEBUG',False),
            no_save == '',
            template != 'questionnaire_feedback.html',
            not request.user.is_staff
            ]
    if all(use_cache_conditions):
        use_cache = True
    ...
    resp =  render_to_response(template,
                             {'form_list': form_list,
                              ...
                              'no_save' : no_save,
                              'DEBUG' : False,#getattr(settings,'DEBUG',False),
                              'CITIES_WITH_ZOOMABLE_DISTRICTS' : getattr(settings,'CITIES_WITH_ZOOMABLE_DISTRICTS',''),
                              'ZOOMABLE_PLACES' : getattr(settings,'ZOOMABLE_PLACES',''),
                              ...
                              'ADD_FEATURE_MIN_ZOOM_LEVEL' : getattr(settings,'ADD_FEATURE_MIN_ZOOM_LEVEL',0),
                              'lottery' : lottery,
